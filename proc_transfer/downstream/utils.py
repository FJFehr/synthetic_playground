...
def initialize_model(
    ...
    shuffle_attn_only: bool = False,
    shuffle_all_weights: bool = False,
    shuffle_seed: int = 42,
    ...
    Args:
        ...
        shuffle_attn_only (bool): After loading, shuffle only the attention weights/biases
            per-component (preserves each component's mean/std, destroys its structure).
        shuffle_all_weights (bool): After loading, shuffle all non-embedding weights/biases
            per-component. Mutually exclusive with shuffle_attn_only.
        shuffle_seed (int): Seed for deterministic shuffling.
    ...
    # Optionally scramble the (just-loaded) weights per-component as a control for
    # whether transfer benefit comes from learned structure vs weight statistics.
    if shuffle_attn_only and shuffle_all_weights:
        raise ValueError(
