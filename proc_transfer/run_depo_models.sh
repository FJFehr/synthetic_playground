#!/bin/bash
# Queue the new models-folder DEPO experiment across 8 GPUs.
#
# 5 pretrained checkpoints x 3 finetuning seeds = 15 DEPO runs.
...
#
# Usage:
#   bash run_depo_models.sh
#   bash run_depo_models.sh <wandb_project>

set -euo pipefail
cd "$(dirname "$0")"

PROJECT="${1:-synthetic_playground}"
LOG_DIR="logs/depo_models"
mkdir -p "$LOG_DIR"
...
run_job() {
  ...
  local ckpt="$3"
  local seed="$4"
  local run_name="depo_${model_id}_seed${seed}"
  local log="$LOG_DIR/${run_name}.log"

  if [ -f "$log" ] && grep -q "Done. Results in" "$log"; then
  ...
  WANDB_RUN_GROUP="$model_id" CUDA_VISIBLE_DEVICES="$gpu" \
    bash run_depo.sh "$model_id" "$ckpt" "$seed" "$PROJECT" "$run_name" \
    > "$log" 2>&1
}
...
echo "Queueing ${#JOB_MODEL_IDS[@]} DEPO runs across ${#GPUS[@]} GPUs (project: $PROJECT)..."
...
echo "All DEPO model-folder queued runs complete."
